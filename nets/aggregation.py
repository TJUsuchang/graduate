        ...
        for i in range(self.num_scales):
            if i == 0:
                ...
                self.conva.append(nn.Sequential(nn.Conv2d(16, 32, kernel_size=1, bias=False),
                                                nn.BatchNorm2d(32),
                                                nn.LeakyReLU(0.2, inplace=True)))
                self.convaa.append(nn.Sequential(nn.Conv2d(16, 32, kernel_size=1, bias=False),
                                                 nn.BatchNorm2d(32),
                                                 nn.LeakyReLU(0.2, inplace=True)))
    ...
    def forward(self, x):
        ...
        for i in range(2, -1, -1):
            if i == 2:
                ...
                exchange1 = self.conva[-1](exchange1)
                ...
                exchange1 = F.interpolate(x_fused[-1], scale_factor=2,
                                          mode='bilinear', align_corners=False)
                exchange1 = self.convb[-1](exchange1)
                exchange2 = F.interpolate(x[1], scale_factor=2,
                                          mode='bilinear', align_corners=False)
                exchange2 = self.convbb[-1](exchange2)
                exchange3 = F.interpolate(x[2], scale_factor=4,
                                          mode='bilinear', align_corners=False)
                exchange3 = self.convaaa[-1](exchange3)
                x_atten.append(self.fuse_layers2[i](x[i]))
